#!/usr/bin/env node
/**
 * lampaimprobe.mjs — IS THE LAMP HEAD OVER THE THING IT LIGHTS? NOT A GATE, and it
 * must never become one. SESSION 68, ITEM 2.
 *
 *   node tools/lampaimprobe.mjs
 *   node tools/lampaimprobe.mjs --at=2048,40,-400
 *
 * ════════════════════════════════════════════════════════════════════════════
 * WHY IT EXISTS — AND WHY IT IS NOT CALLED `lampprobe`
 * ════════════════════════════════════════════════════════════════════════════
 *
 * `tools/lampprobe.mjs` HAS EXISTED SINCE SESSION 23 and asks a completely
 * different question — *"does a tree crown grow into a lamp head, and how
 * often"*. This session wrote over it and had to restore it from `c05d27e`;
 * the name is taken and the second tool takes a second name. A probe called
 * `lampprobe` that measured aim rather than canopy clash would have been the
 * §9 defect this project has seventy-five rows of, with a filename instead of
 * a float.
 *
...
try {
  ...
  console.log(`lampaimprobe — NOT A GATE.\n\n  GPU: ${await readRendererString(page)}`);
